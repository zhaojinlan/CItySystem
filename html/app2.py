...
def recognize_city_entity(text: str, cities: List[Dict], use_model: bool = False) -> Optional[str]:
    """
    改进版城市实体识别（支持精确匹配、模型增强）

    参数：
    text: 待识别文本
    cities: 城市字典列表，需包含'name'键
    use_model: 是否启用模型辅助识别（需实现模型部分）
    ...
    # 改进1：正则精确匹配（避免部分匹配）
    pattern_cache = {city['name']: re.compile(rf'\b{re.escape(city["name"])}\b') for city in sorted_cities}
    ...
    for city in sorted_cities:
        ...
        # 策略1：正则全词匹配
        if pattern_cache[name].search(text):
            return name

        # 策略2：分词匹配（可选）
        # if any(name == word for word in words):
        #     return name

        # 策略3：模型结果交叉验证
        if use_model and name in model_matches:
            return name

        # 策略4：拼音匹配（需安装pypinyin）
        # if ''.join(lazy_pinyin(name)) in ''.join(lazy_pinyin(text)):
        #     return name
...
def ner_qa():
    ...
    # 实体识别阶段
    city = recognize_city_entity(user_input, city_names)
